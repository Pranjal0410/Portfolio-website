...
function Hero() {
  return(<section id="home" className="relative flex items-center justify-center px-4 overflow-hidden z-10" style={{minHeight:'100vh',paddingTop:'7rem',paddingBottom:'4rem'}}><div className="relative z-10 w-full max-w-3xl mx-auto text-center">
    <AS><div className="flex justify-center mb-6"><img src="/profile.png" alt="Pranjal Udhwani" className="w-32 h-32 sm:w-40 sm:h-40 rounded-full object-cover" style={{filter:'drop-shadow(0 0 40px rgba(139,92,246,0.3))'}}/></div></AS>
    <AS delay={100}><h1 className="text-4xl sm:text-5xl md:text-[68px] font-extrabold tracking-tighter leading-[1] mb-3" style={{letterSpacing:'-0.03em'}}>Hi, I'm <span className="shimmer-text">Pranjal Udhwani</span></h1><p className="text-lg sm:text-xl md:text-[21px] text-zinc-400 font-medium tracking-tight mb-5">I build production-grade software. AI-powered. Stress-tested at scale.</p></AS>
    <AS delay={250}><p className="text-lg sm:text-xl text-zinc-400 max-w-xl mx-auto mb-2" style={{lineHeight:1.8}}><Typewriter lines={["I don't build demos. I build things people actually use.","Most devs stop at localhost. I don't.","Full-stack. AI-native. Shipped before graduation."]} speed={38}/></p><p className="text-sm text-zinc-600 mt-2">Open to internships & freelance</p></AS>
    <AS delay={400}><div className="flex flex-col sm:flex-row items-center justify-center gap-4 mt-10"><a href="mailto:udhwanipranjal@gmail.com?subject=Let's%20connect" className="btn-primary">Let's connect <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6"/></svg></a><a href="#projects" className="btn-secondary">See projects <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M19 14l-7 7m0 0l-7-7m7 7V3"/></svg></a></div></AS>
