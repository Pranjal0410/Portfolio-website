...
function SkillGlobe() {
  ...
  return(
    <div className="relative w-full flex items-center justify-center select-none" style={{height:580,cursor:dragging.current?'grabbing':'grab'}} onMouseDown={onDown} onMouseMove={onMove} onTouchStart={onDown} onTouchMove={onMove}>
      <svg width={R*2+100} height={R*2+100} className="absolute"><circle cx={R+50} cy={R+50} r={R} fill="none" stroke="rgba(139,92,246,0.06)" strokeWidth="0.5"/>{wires}</svg>
      <div className="absolute w-24 h-24 bg-violet-500/12 rounded-full blur-[40px]"/>
      {positioned.map((ic,i)=>{const s=0.5+(ic.z+R)/(2*R)*0.55,o=0.15+(ic.z+R)/(2*R)*0.85;return(<div key={i} className="absolute pointer-events-none" style={{left:`calc(50% + ${ic.x}px)`,top:`calc(50% + ${ic.y}px)`,transform:`translate(-50%,-50%) scale(${s})`,opacity:o,zIndex:Math.round(ic.z+R)}}><div className="px-3 py-1.5 rounded-xl border border-white/[0.08] backdrop-blur-sm font-mono font-semibold text-[11px] whitespace-nowrap" style={{background:`${ic.c}10`,color:ic.c,boxShadow:`0 0 12px ${ic.c}08`}}>{ic.n}</div></div>)})}
    </div>
  )
}
